...
def micro_evaluate(batch, pred_ner, micro_eva_dic):
    for sample, preds in zip(batch, pred_ner):
        for gold, pred in zip(sample['spans_label'], preds):
            if gold == 1:
                micro_eva_dic[1]['tot_gold'] += 1
                cor, tot_pred = one_eval(gold, pred)
                micro_eva_dic[1]['cor'] += cor
                micro_eva_dic[1]['tot_pred'] += tot_pred
            elif gold == 2:
                micro_eva_dic[2]['tot_gold'] += 1
                cor, tot_pred = one_eval(gold, pred)
                micro_eva_dic[2]['cor'] += cor
                micro_eva_dic[2]['tot_pred'] += tot_pred
            elif gold == 3:
                micro_eva_dic[3]['tot_gold'] += 1
                cor, tot_pred = one_eval(gold, pred)
                micro_eva_dic[3]['cor'] += cor
                micro_eva_dic[3]['tot_pred'] += tot_pred
            elif gold == 4:
                micro_eva_dic[4]['tot_gold'] += 1
                cor, tot_pred = one_eval(gold, pred)
                micro_eva_dic[4]['cor'] += cor
                micro_eva_dic[4]['tot_pred'] += tot_pred
    return micro_eva_dic
...
def evaluate(model, batches, tot_gold):
    ...
    wrong_span = 0
    # micro_eva_dic = {
    #     1: {'cor': 0, 'tot_pred': 0, 'tot_gold': 0},
    #     2: {'cor': 0, 'tot_pred': 0, 'tot_gold': 0},
    #     3: {'cor': 0, 'tot_pred': 0, 'tot_gold': 0},
    #     4: {'cor': 0, 'tot_pred': 0, 'tot_gold': 0},
    # } # just for conll03
    ...
    for i in trange(len(batches)):
        ...
        pred_ner = output_dict['pred_ner']
        # ---- micro evaluation -----
        # micro_eva_dic = micro_evaluate(batches[i], pred_ner, micro_eva_dic)
        # ---- micro evaluation -----
    ...
    # ---- micro evaluation -----
    # for key in micro_eva_dic:
    #     cor = micro_eva_dic[key]['cor']
    #     tot_pred = micro_eva_dic[key]['tot_pred']
    #     tot_gold = micro_eva_dic[key]['tot_gold']
    #     p = cor / tot_pred if cor > 0 else 0.0
    #     r = cor / tot_gold if cor > 0 else 0.0
    #     f1 = 2 * (p * r) / (p + r) if cor > 0 else 0.0
    #     logger.info('Label %d: P: %.5f, R: %.5f, F1: %.5f'%(key, p, r, f1))
    # ---- micro evaluation -----
...
if __name__ == '__main__':
    ...
    parser.add_argument('--base_model', type=str, default='roberta_ns', required=True, choices=['roberta_ns', 'roberta', 'bert', 'bert_ns'])
    ...
    if args.NO_NS:
        logger.info('===> Without Negative Sampling...')
    else:
        logger.info('===> With Negative Sampling...')
    ...
    if args.do_train:
        ...
        if args.dm_filter:
            ...
            logger.info("!!!Changing Used datasets: %s"%new_train_samples_path)
