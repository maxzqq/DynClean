...
def noise_filter(args, train_samples, td_df):
    """
    Filter the noise positive/negative samples by training dynamics.
    """
    # get the guids of pos/neg
    pos_guids = []
    neg_guids = []
    for sample in train_samples:
        ...
        for i in range(len(spans_label)):
            if spans_label[i] != 0:
                pos_guids.append(guids[i])
            else:
                neg_guids.append(guids[i])
    # for negatives
    neg_df = td_df[td_df['guid'].isin(neg_guids)]
    sorted_neg_df = neg_df.sort_values(by='confidence', ascending=True)
    selected_neg_df = sorted_neg_df.head(n=int(args.neg_rate * len(sorted_neg_df)))
    neg_indices = list(selected_neg_df['guid'])
    # for positives
    pos_df = td_df[td_df['guid'].isin(pos_guids)]
    sorted_pos_df = pos_df.sort_values(by='confidence', ascending=False)
    selected_pos_df = sorted_pos_df.head(n=int(args.pos_rate * len(sorted_pos_df)))
    pos_indices = list(selected_pos_df['guid'])
    # combine the indices
    indices = neg_indices + pos_indices
    # filter the samples
    logger.info("Starting filtering the training samples by training dynmaics...")
    logger.info("Before filtering, the number of span samples is %d"%len(train_samples))
    new_samples = []
    num_sample = 0
    for i in trange(len(train_samples)):
        ...
        train_samples[i]['guids'] = new_guids
        train_samples[i]['spans'] = new_spans
        train_samples[i]['spans_label'] = new_spans_label
        if len(train_samples[i]['spans']) > 0:
            num_sample += len(train_samples[i]['spans'])
            new_samples.append(train_samples[i])
    logger.info("After filtering, the number of span samples is %d"%num_sample)
    return new_samples
...
if __name__ == '__main__':
    ...
    # TD temp train dataset
    args.td_temp_dataset = os.path.join(args.data_dir, 'td_temp_dataset.json')
    ...
    for iter in tqdm(range(args.num_iteration)):
        logger.info('======================> Start Iteration %d'%iter)
        # training dynamics dataframe list
        TD_df = []
        # initial the model
        ...
        if args.do_train:
            ...
            if iter == 0:
                ...
                # write down the initial train dataset as the temp train dataset
                # with open(args.td_temp_dataset, 'w') as f:
                #     json.dump(train_samples, f)
            else:
                train_samples = new_samples # the new_samples are generated by the last iteration
            

            if args.dm_filter:
                # train_samples = td_filter(train_samples, td_path, args.dm_rate)
                new_train_samples_path = os.path.join(args.data_dir, 'td_temp_dataset.json')
        ...
        # After one iteration,  and update the training dataset
        # 1. compute the training dynamics
        #   1.1 compute the confidence of each sample
        #   1.2 save the training dynamics into a jsonl file
        # 2. update the training dataset
        #   2.1 filter the samples with low confidence
        #   2.2 add the new samples into the training dataset
        train_dy_metrics  = compute_td(TD_df, args.num_epoch)
        train_dy_filename = os.path.join(args.dm_path, f"td_df_pos{args.pos_rate}_neg{args.neg_rate}_iter{iter}.jsonl")
        train_dy_metrics.to_json(train_dy_filename,
                            orient='records',
                            lines=True)
        logger.info(f"Metrics based on Training Dynamics written to {train_dy_filename}")
        # new_samples = noise_filter(args, train_samples, train_dy_metrics)
        # logger.info("Noise Filtered by positive rate %.4f and negative rate %.4f"%(args.pos_rate, args.neg_rate))
        # # saving the new samples into the output_dir
        # temp_dataset_path = os.path.join(args.dm_path, f'td_dataset_pos{args.pos_rate}_neg{args.neg_rate}_iter{iter}.json')
        # with open(temp_dataset_path, 'w') as f:
        #     json.dump(new_samples, f)
        # logger.info("New samples saved to %s"%temp_dataset_path)
