...
for i in trange(len(samples)):
    ...
    for j in range(len(guids)):
        ...
        if guid in aum_guids:
            ...
            # get the new label_id
            label_ids = [i for i in range(num_ner_labels+1)]
            label_ids.remove(original_label)
            new_label_id = random.choice(label_ids)
...
# save the data
with open(f'/home/qi/Projects/NER/ds_ner/data/{dataset_name}/aum/train-ds-samples-aum-new-method3.json', 'w') as f:
    json.dump(samples, f)
with open(f'/home/qi/Projects/NER/ds_ner/data/{dataset_name}/aum/fake_class_guids-new-method3.pkl', 'wb') as f:
