...
df = pd.read_json(f'/home/qi/Projects/NER/ds_ner/out/rb_{dataset_name}/dy_log/td_df.jsonl', lines=True)
# 1. 先处理Negatives
# 获取hard negatives
print("===> For Negatives")
...
aum = pd.read_csv(f'/home/qi/Projects/NER/ds_ner/out/rb_{dataset_name}_aum/dy_log/aum_values.csv')
...
# 真实的AUM
aum = pd.read_csv(f'/home/qi/Projects/NER/ds_ner/out/rb_{dataset_name}/dy_log/aum_values.csv')
...
print(len(rm_hard_neg_guids), len(hard_neg_aum))
inter_sec = list(set(rm_hard_neg_guids).intersection(incomplete_guids))
print("删掉的hard negatives数目:", len(inter_sec))
...
with open(f"/home/qi/Projects/NER/ds_ner/data/{dataset_name}/train-ds-samples-removed-noise.json", "w") as f:
        json.dump(new_ds, f)
...
with open(f'/home/qi/Projects/NER/ds_ner/data/{dataset_name}/train-ds-samples-noise-flag.json', 'w') as f:
    json.dump(ds_data, f)
