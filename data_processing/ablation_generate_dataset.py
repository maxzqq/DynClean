...
df = pd.read_json(f'/home/qi/Projects/NER/ds_ner/out/ab_{dataset}_rb_2/dy_log/td_df.jsonl', lines=True)
rates = [0.01]
for rate in rates:
    neg_df = df[df['guid'].isin(non_entity_guids)]
    neg_df = neg_df.sort_values(by=['confidence'], ascending=True)
    selected_df = neg_df.head(n=int(rate * len(neg_df)))
    hard_neg = list(selected_df['guid'])
    hard_rm = list(set(hard_neg).intersection(incomplete_guids))
    print(f"======== {rate}=========")
    print(len(hard_rm), len(hard_neg))
# 1. 先处理Negatives
# 获取hard negatives
print("===> For Negatives")
rates = [0.33]
for rate in rates:
    ...
    hard_neg = list(selected_df['guid'])
    hard_rm = list(set(hard_neg).intersection(incomplete_guids))
    print(f"========Hard Neg {rate}=========")
    print(len(hard_rm), len(hard_neg))

# 获取对应的fake aum threshold
with open(f'/home/qi/Projects/NER/ds_ner/data/{dataset}/aum/fake_class_guids.pkl', 'rb') as f:
    aum_guids = pickle.load(f)
aum = pd.read_csv(f'/home/qi/Projects/NER/ds_ner/out/ab_{dataset}_aum_rb_2/dy_log/aum_values.csv')
...
thre_aum = fake_aum.iloc[int(len(fake_aum)*0)]['aum']
print("Negatives AUM threshold: ", thre_aum)

# 获得需要被删掉的hard negatives的guids
# 真实的AUM
aum = pd.read_csv(f'/home/qi/Projects/NER/ds_ner/out/ab_{dataset}_rb_2/dy_log/aum_values.csv')
...
print("删掉的False negatives中error的数目:", len(inter_sec))
...
selected_df = pos_df.head(n=int(0.33 * len(pos_df)))
...
fake_aum = fake_aum.sort_values(by=['aum'], ascending=False)
# thre_aum = fake_aum.iloc[0]['aum']
thre_aum = fake_aum.iloc[int(len(fake_aum)*0.2)]['aum']
print("Positive AUM threshold: ", thre_aum)

# 获得需要被删掉的hard positives的guids
# 真实的AUM
aum = pd.read_csv(f'/home/qi/Projects/NER/ds_ner/out/ab_{dataset}_rb_2/dy_log/aum_values.csv')
hard_pos_aum = aum[aum['sample_id'].isin(hard_pos)]
...
print("删掉的总的 positives数目:",len(rm_hard_pos_guids))
inter_sec = list(set(rm_hard_pos_guids).intersection(inaccurate_guids))
print("删掉的False positives数目:", len(inter_sec))
...
with open(f"/home/qi/Projects/NER/ds_ner/data/{dataset}/train-ds-samples-removed-noise-no-ns-3.json", "w") as f:
        json.dump(new_ds, f)
